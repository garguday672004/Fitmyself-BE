...
async function getUserProfile(auth) {
  ...
  console.log(profile.data.names[0].displayName);
...
app.get("/auth/google", (req, res) => {
  ...
  // hard code me to the authUrl link in response
  res.json({ authUrl });
});
...
app.get("/auth/google/callback", async (req, res) => {
  ...
  try {
    ...
    const userId = req.session.userProfile.userID;
    ...
    // Fetch the existing login counter document from the database
    let loginCounterDoc;
    try {
      loginCounterDoc = await database.getDocument(
        process.env.DATABASE_ID,
        process.env.LOGIN_COUNTER_COLLECTION_ID,
        uuId
      );
    } catch (error) {
      // Handle the error if the document is not found
      console.error("Document not found:", error);
      // Set loginCounterDoc to null or any default value as needed
      ...
      try {
        ...
        console.log("Login counter is : ", loginCounter);
      } catch (error) {
        console.error("Error in creating new doc", error);
        // Handle error
      }
    }
    ...
    // If the login counter document exists, retrieve the count value
    if (loginCounterDoc) {
      loginCounter = loginCounterDoc.count;
    }

    // Increment the login counter
    loginCounter++;

    // Save the updated login counter to the database
    try {
      if (loginCounterDoc) {
      ...
      console.error("Error updating login counter:", error);
      // Handle error
    }
  } catch (error) {
  ...
  // Your existing code for handling Google OAuth and fetching user profile

  // Redirect the user to the dashboard or any other route
  res.redirect("http://localhost:3000/dashboard");
});

app.get("/login-counter", async (req, res) => {
    const profile = await getUserProfile(oAuth2Client);
    // Save user profile data in the session
    userProfileData = profile;
    // console.log("User profile data:", profile);
  ...
  // Retrieve the login counter from the database
  try {
    const loginCounterDoc = await database.getDocument(
      process.env.DATABASE_ID,
      process.env.LOGIN_COUNTER_COLLECTION_ID,
      uuId
    );
    ...
    console.error("Error retrieving login counter:", error);
    // Handle error
  }

  console.log("Login counter:", loginCounter);
  // Send the login counter to the frontend
  res.json({ count: loginCounter, userID: uuId});
});
...
//    //res.redirect("http://localhost:8000/fetch-data");
//  catch (error) {
//   console.error("Error retrieving access token:", error);
//   res.redirect("/error");
// }
